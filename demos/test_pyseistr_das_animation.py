...
plt.imshow(dn,cmap=seis(),clim=(-clip,clip),aspect='auto');plt.gca().set_xticks([]);plt.gca().set_yticks([]);plt.savefig('dn.png')
plt.imshow(d1_bpsomffk,cmap=seis(),clim=(-clip,clip),aspect='auto');plt.gca().set_xticks([]);plt.gca().set_yticks([]);plt.savefig('d1.png')
from pyseistr import wiping
wiping(['dn.png','d1.png'], 'test_pyseistr_das_animation.gif', duration=500, number=15)
